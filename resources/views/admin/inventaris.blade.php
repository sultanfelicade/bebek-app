	...
	<div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
		<div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
			<div class="p-6 border-b border-gray-100">
				<h2 class="text-lg font-bold text-gray-900">Stok Ringkas</h2>
			</div>
					...
					<tbody class="divide-y divide-gray-100">
						@forelse ($stocks as $stock)
							<tr class="hover:bg-gray-50 transition-colors">
								...
								<td class="px-6 py-4 text-right text-gray-900 font-semibold">{{ number_format((float) $stock->stock) }}</td>
							</tr>
						@empty
							<tr><td colspan="2" class="px-6 py-12 text-center text-gray-400">Belum ada stok.</td></tr>
						@endforelse
					</tbody>
				</table>
			</div>
		...
		<div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
			<div class="p-6 border-b border-gray-100">
				<h2 class="text-lg font-bold text-gray-900">Mutasi Terbaru</h2>
			</div>
				...
				@forelse ($mutations as $mutation)
					<div class="px-6 py-4 hover:bg-gray-50 transition-colors">
						<div class="flex items-center justify-between gap-3">
							<div>
								<p class="font-semibold text-gray-900">{{ $mutation->product_name }}</p>
								<p class="text-xs text-gray-500 mt-0.5">{{ $mutation->created_at ?? '-' }}</p>
							</div>
							<div class="text-right">
								<span class="inline-flex px-2.5 py-1 rounded-full text-xs font-bold {{ strtoupper($mutation->type) === 'IN' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
									{{ strtoupper($mutation->type) }} {{ number_format((float) $mutation->qty) }}
								</span>
							</div>
						</div>
						@if ($mutation->note)
							<p class="mt-1 text-sm text-gray-500">{{ $mutation->note }}</p>
						@endif
					</div>
				@empty
					<div class="px-6 py-12 text-center text-gray-400">Belum ada mutasi stok.</div>
				@endforelse
			</div>
		</div>
	</div>
