...
@section('content')
    <form method="POST" action="{{ url('/master/products/' . $product->id_product) }}" class="rounded-3xl border border-slate-100 bg-white shadow-sm p-5 space-y-4 max-w-2xl">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-sm text-slate-500 mb-1">Nama Menu</label>
            <input name="product_name" value="{{ old('product_name', $product->product_name) }}" required class="w-full rounded-lg bg-slate-950 border border-gray-300 px-3 py-2 text-slate-900">
        </div>
        <div>
        ...
        <div>
            ...
            <input name="price" type="number" step="0.01" min="0" value="{{ old('price', $product->price) }}" required class="w-full rounded-lg bg-slate-950 border border-gray-300 px-3 py-2 text-slate-900">
        </div>
