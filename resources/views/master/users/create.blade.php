...
@section('content')
    <form method="POST" action="{{ url('/master/users') }}" class="rounded-3xl border border-slate-100 bg-white shadow-sm p-5 space-y-4 max-w-2xl">
        @csrf
        <div>
            <label class="block text-sm text-slate-500 mb-1">Username</label>
            <input name="username" value="{{ old('username') }}" required class="w-full rounded-lg bg-slate-950 border border-gray-300 px-3 py-2 text-slate-900">
        </div>
        <div>
            <label class="block text-sm text-slate-500 mb-1">Password</label>
            <input name="password" type="password" required class="w-full rounded-lg bg-slate-950 border border-gray-300 px-3 py-2 text-slate-900">
        </div>
        <div>
